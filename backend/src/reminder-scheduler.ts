...
export class ReminderScheduler {
  ...
  private async deliver(x: { now: Date; workspaceId: string; cardId: string; card: Record<string, unknown>; statement: StatementDto; days: number; user: Awaited<ReturnType<AuthRepository["findUserById"]>>; amount: number }) {
    ...
    try { await this.mail.sendPaymentReminder(composePaymentReminder({ to: x.user.email.toLowerCase(), cardName: String(x.card.displayName ?? "Thẻ tín dụng"), statementDate: x.statement.statementDate, dueDate: x.statement.paymentDueDate, amount: x.amount, daysBefore: x.days })); await ReminderDeliveryModel.updateOne({ _id: delivery._id }, { $set: { status: "SENT", sentAt: x.now, failureCode: null } }); this.log.info({ event: "PAYMENT_REMINDER_SENT", deliveryId: String(delivery._id) }); }
