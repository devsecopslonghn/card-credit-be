...
export const cardDtoFromDocument = (value: unknown): CardDto => {
  ...
  return cardPortfolioCardSchema.parse({
    ...
    providerCode: stringOrNull(card.providerCode),
    providerName: stringOrNull(card.providerName),
    displayName: stringOrNull(card.displayName),
    network: stringOrNull(card.network),
    owner: stringOrNull(card.owner),
